...
- [ ] T067 [US3] Implement the ATS submitter — **RE-SCOPED 2026-09-10, no longer an ATS submitter; scaffolding built, not yet functional.** Originally planned as `api/src/apply/submit-ats.ts`, allowlist-gated to ATS platforms with CAPTCHA/multi-step detection. **PAUSED 2026-09-01, not attempted**: verified Greenhouse's own documented submission endpoint requires a private, employer-issued API key (Basic Auth) — no ATS platform has a genuinely public submission channel a third-party candidate app can use (`BLOCKERS.md`, `SDD.md` R13). **2026-09-10**: Product explicitly chose `BLOCKERS.md`'s option 3 — build browser-automation submission, accepting the risk — scoped to JobsDB HK's/104.com.tw's own native apply flow (not ATS platforms; current sourcing doesn't pull ATS-hosted postings anymore, see T046/T088 notes) and gated to one personal-test profile. Implemented as `api/src/browser-agent/` (orchestrator, session-state capture/encryption, per-provider drivers) plus `sourcing/apply-route.ts`'s new `resolveApplyRoute`. `providers/jobsdb-hk.ts` deliberately throws until its selectors are verified live by hand (constitution Principle VI — see that file's module comment); `providers/tw104.ts` is a stub, deferred until then. See `compliance/risk-acceptance-log.md`'s 2026-09-10 entry for the full scope and what it does/doesn't cover. **2026-09-21**: [cua](https://github.com/trycua/cua) confirmed as a second, available attempt-tool alongside Playwright-stealth for this same scope when the selectors do get verified — not an expansion of target/rollout, and not an escalation triggered by Playwright-stealth failing (bot walls still hard-fail to `needs_attention`). See `BLOCKERS.md` and `compliance/risk-acceptance-log.md`'s 2026-09-21 entries.
...
- [X] T090 [P] Update `CLAUDE.md` and add `api/README.md` + `ios/README.md` with setup and architecture pointers
  - **Build status**: Done. Root `CLAUDE.md` rewritten — it predated the Fyndra rename, the career-ops-as-design-reference-only decision, and effectively all backend work, and still described "an iOS app (product direction TBD)". Now reflects the real product, the two codebases, points at `tasks.md`/`BLOCKERS.md`/`SDD.md`, and adds the "verify before building" and "log, don't stall" orchestrator rules that were established through this session's actual incidents (JobsDB HK, T067). `api/README.md`'s "not yet implemented" section was stale (all four user stories are done) — replaced with an accurate "what exists today" summary plus notes on the OpenRouter free-tier flake and the handoff-only apply route. `ios/README.md` was already accurate (no Xcode project is still the real state) and needed no change.
- [ ] T091 Run the full [quickstart.md](./quickstart.md) validation end-to-end on a physical device over ngrok
- [X] T092 Run `/speckit-constitution` to add backend engineering principles — the constitution is iOS-only and this feature ships a server (SDD R7)
  - **Build status**: Done. `.specify/memory/constitution.md` bumped 1.0.0 → 1.1.0 (MINOR — new principle, no removal). Added "VI. Backend Engineering Standards (NON-NEGOTIABLE)" codifying conventions this codebase already follows: real-Postgres TDD (never mock the ORM), the 80/70/80/80 coverage gate already enforced in `api/jest.config.js`, verify-external-APIs-before-building (the rule this session adopted after the JobsDB HK and T067 incidents), untrusted-text-is-data-never-instructions for LLM-touching code (SDD §10.1), and no-secrets-in-version-control. Added three matching backend Quality Gate lines. Also resolved TODO(PROJECT_NAME) (Fyndra, internal use only pending trademark clearance) and TODO(MIN_IOS_VERSION) (iOS 17+) while in the file — both were stale placeholders with known answers. TODO(UIUX_STANDARDS) and TODO(TOOLCHAIN) left as-is, still genuinely undecided.
- [ ] T093 [US3] **Sharpened 2026-09-21, ahead of Architecture Board review — not started.** Route the fixed-`values` (choice-field) branch of `apply/prefill.ts`'s `draftAnswer()` through a new in-house `ChoiceClient` (typed `{choice, probabilities, confidence}`, the `Choice`/`Score`/`Noul` contract reimplemented against the existing `LlmClient` provider — not TypeSafe AI's hosted Jev, which turned out to be waitlist-only with no self-host option) instead of the current free-text-completion + exact-label string-match. Free-text fields stay on `LlmClient`; `apply/sensitive.ts` gating is unaffected. Confidence threshold 70%, explicitly provisional; fails closed to `pending_needs_answer` on error. See `research.md`'s "Structured answer drafting: the Choice/Score/Noul contract" entry, `SDD.md` §6.7, and `BLOCKERS.md` 2026-09-21 for full rationale (depends on T065).
