...
description: "Task list for Swipe2Work — swipe-to-apply job search (HK/TW), Phase 1"
---

# Tasks: Swipe2Work — Swipe-to-Apply Job Search (HK/TW)
...
Mobile + API per [plan.md](./plan.md): `api/src/`, `api/tests/`, `ios/Swipe2Work/`, `ios/Swipe2WorkTests/`
...
- [ ] T029 [P] [US1] Snapshot tests for the profile-review screen in `ios/Swipe2WorkTests/Snapshot/ProfileReviewTests.swift` — **including zh-Hant at the largest Dynamic Type size** (constitution III)
...
- [ ] T036 [P] [US1] Implement the CV upload view + view model in `ios/Swipe2Work/Features/Profile/` with document picker, all four states (loading/empty/error/success)
- [ ] T037 [US1] Implement the keyword/YoE review-and-edit view in `ios/Swipe2Work/Features/Profile/` (depends on T036)
...
- [ ] T043 [P] [US2] Snapshot tests for the swipe card and end-of-feed state in `ios/Swipe2WorkTests/Snapshot/JobFeedTests.swift` — zh-Hant titles at large Dynamic Type must not clip
- [ ] T044 [P] [US2] UI test for the swipe gestures in `ios/Swipe2WorkTests/UITests/SwipeFeedUITests.swift` (constitution II: critical flow)
...
- [ ] T055 [P] [US2] Implement the swipe deck view + view model in `ios/Swipe2Work/Features/JobFeed/` with card pre-fetching so a swipe never awaits the network
- [ ] T056 [US2] Implement the end-of-feed and broaden-criteria states in `ios/Swipe2Work/Features/JobFeed/` (depends on T055)
...
- [ ] T062 [P] [US3] Snapshot tests for the answer-sheet review and pending-question screens in `ios/Swipe2WorkTests/Snapshot/ApplyTests.swift`
...
- [ ] T071 [P] [US3] Implement the answer-sheet review UI in `ios/Swipe2Work/Features/ApplicationTracking/` — per-field proposed answer, source label, inline edit
- [ ] T072 [P] [US3] Implement the pending-question UI in `ios/Swipe2Work/Features/ApplicationTracking/`, explaining *why* a sensitive question must be answered by the user
- [ ] T073 [US3] Implement the submission-mode setting and cap display in `ios/Swipe2Work/Features/Settings/` — default review-before-sending, auto-submit as an informed opt-in (FR-018, FR-019)
...
- [ ] T076 [P] [US4] Snapshot tests for the tracking list and detail timeline in `ios/Swipe2WorkTests/Snapshot/TrackingTests.swift`
...
- [ ] T080 [P] [US4] Implement the tracking list view + view model in `ios/Swipe2Work/Features/ApplicationTracking/`
- [ ] T081 [US4] Implement the application detail timeline and the status-update control in `ios/Swipe2Work/Features/ApplicationTracking/` (depends on T080)
- [ ] T082 [US4] Implement push-permission request at first right-swipe (contextual, not at launch) and in-app badge fallback in `ios/Swipe2Work/` (constitution V)
...
- [ ] T083 [P] Add `ios/Swipe2Work/PrivacyInfo.xcprivacy` and the in-app privacy disclosure per T003's findings (constitution V)
