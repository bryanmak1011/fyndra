...
| D2 | **Nobody hands us a submitter.** career-ops refuses to submit by design | `web/src/lib/apply/session.ts`: *"NEVER clicks a submit/apply control"*; `LEGAL_DISCLAIMER.md` §5 lists auto-submit as unacceptable use | We build it, allowlist-gated to ATS with published schemas (§6.5). Its refusal is still a **signal**: the reason it refuses is our risk too (§12 R1). **Update 2026-09-10**: no ATS ever had a public submission channel to allowlist against (R13) — as of `compliance/risk-acceptance-log.md`'s 2026-09-10 entry, we now do exactly what career-ops refuses (click a real submit control), scoped to one personal-test profile against JobsDB HK/104.com.tw's own apply flow. See `browser-agent/`. |
...
| # | Risk | Sev | Mitigation |
|---|---|---|---|
| R1 | **App Store rejection** for automating third-party services / employment-application handling | High | Auto-submit restricted to allowlisted public ATS endpoints with no credential custody and no ToS-prohibited platform (§6.5, §10.1); volume caps (§6.5.3); no LinkedIn/Indeed automation at all (D2). Guideline 5.2.2 requires proof of authorization on request; **Phase 0 finding (2026-08-31, `compliance/app-store-assessment.md`)**: confirmed for the *reading* side (documented Job Board APIs), but the *submission* side needs a per-ATS terms confirmation before T067 — mitigation is conditional, not fully closed. **Update 2026-09-10 (`compliance/risk-acceptance-log.md`)**: T067 is now built as browser-automated submission against JobsDB HK's/104.com.tw's native apply flow, personal-test-profile-gated only (`browser-agent/gate.ts`) — the automation itself runs as a laptop/server-side worker process, never inside the iOS client binary, which keeps this specific build outside Guideline 5.2.2's on-device-automation framing for now. The broader product-level exposure (submitting real third-party employment applications on a user's behalf at all) is deferred, not closed — re-assess before any general (non-personal-test) rollout. |
...
| R13 | **No ATS has a genuinely public *submission* API either — not even Greenhouse.** The entire "auto-submit via clean API calls" premise (D2, §6.5) is unconfirmed for every current provider | **Personal-test browser automation accepted, see `compliance/risk-acceptance-log.md`'s 2026-09-10 entry (was: Critical, build paused, 2026-09-01)** | Checked Greenhouse's own documented submission endpoint directly: it requires HTTP Basic Auth with an API key from the *employer's own* Greenhouse "API Credentials" page — a private, per-organization credential, not the public board token. The only remaining path to real submission on any of the 4 ATS platforms is driving the actual candidate-facing web form — real browser automation, which is exactly what career-ops's design refuses to do (D2) and what this SDD assumed we'd avoid. **Update 2026-09-10**: Product explicitly chose `BLOCKERS.md`'s option 3 (accept browser-automation risk), scoped narrowly — not against Greenhouse/ATS platforms (current sourcing no longer pulls ATS-hosted postings at all, see R4/the 2026-09-10 sourcing pivot), but against JobsDB HK's and 104.com.tw's own native apply flow, gated to exactly one personal-test `UserProfile.id` (`browser-agent/gate.ts`), using a captured Playwright session rather than stored credentials (`browser-agent/session.ts`). `ApplyRoute` gained `browser_automated`; `sourcing/apply-route.ts`'s new `resolveApplyRoute` is the per-Application override that can select it. `providers/jobsdb-hk.ts` is deliberately left unimplemented (throws a descriptive error) pending a live, hands-on DOM-verification pass per the constitution's Principle VI — see that file. `providers/tw104.ts` is a stub, deferred until JobsDB HK is verified end-to-end. |
